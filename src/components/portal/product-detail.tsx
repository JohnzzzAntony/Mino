...
export function PortalProductDetail(props: PortalProductDetailProps) {
  ...
  return <PortalProductDetailInner key={props.productSlug} {...props} />
}

function PortalProductDetailInner({ productSlug }: PortalProductDetailProps) {
  ...
  const casePack = product?.casePackSize ?? 1
  ...
  const handleAddToCart = () => {
    ...
    toast.success('Added to cart', {
      description: `${product.name} · ${qty} ${product.unit}${qty > 1 ? 's' : ''}`,
      action: {
        label: 'View cart',
  ...
  const handleSaveToGuide = () => {
    if (!guideName.trim()) {
      toast.error('Please enter a guide name')
      return
    }
    toast.info('Order Guides coming soon', {
      description: `"${guideName}" will be saved with this product once the API is ready.`,
    })
    setGuideName('')
    setGuideOpen(false)
  }
  ...
  // case-pack aware qty stepper (step by case pack if casePack > 1)
  const step = casePack > 1 ? casePack : 1
  const caseAwareLabel =
    casePack > 1
      ? `${qty} ${product.unit}${qty > 1 ? 's' : ''} · ${Math.max(1, Math.round(qty / casePack))} case${Math.round(qty / casePack) > 1 ? 's' : ''} of ${casePack}`
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {user?.pricingTierName && (
          <div className="mb-6 flex flex-wrap items-center gap-3 rounded-xl border border-primary/20 bg-primary/5 px-4 py-3 text-sm">
            <Building2 className="h-4 w-4 text-primary" />
            <span className="font-medium">{user.companyName ?? 'Your account'}</span>
        ...
        <div className="grid gap-10 lg:grid-cols-2">
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="rounded-xl border border-emerald-200/60 bg-emerald-50/60 p-4 dark:border-emerald-900/60 dark:bg-emerald-950/20">
              ...
              {showOriginal && (
                <p className="mt-1 text-xs text-emerald-700 dark:text-emerald-400">
                  ...
                  {product.discountPercent ? ` · ${product.discountPercent}% tier discount` : ''}
                </p>
              )}
              {casePack > 1 && (
                <p className="mt-1 text-xs text-muted-foreground">
                  ≈ {formatCurrency(effectivePrice * casePack)} per case of {casePack}
                </p>
              )}
            </div>
            ...
            {recycledPct > 0 && (
              <div className="rounded-xl border border-emerald-200/60 bg-emerald-50/60 p-4 dark:border-emerald-900/60 dark:bg-emerald-950/20">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-emerald-700 dark:text-emerald-400">Per case</span>
                </div>
                ...
                <div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs">
                  <div>
                    ...
                    <p className="font-semibold text-foreground">{metrics.treesSavedPerCase ?? 0}</p>
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-foreground">{metrics.plasticSavedLbs ?? 0} lbs</p>
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-foreground">{metrics.waterSavedGal ?? 0} gal</p>
            ...
            {/* Qty + Add to cart + Add to order guide */}
            <div className="flex flex-col gap-3">
              <div className="flex flex-wrap items-stretch gap-3">
                <div className="flex items-center rounded-md border border-input">
                  ...
                  <input
                    ...
                    className="h-10 w-16 border-0 bg-transparent text-center text-sm font-medium outline-none"
                  />
                  <Button
                    variant="ghost"
                    size="icon"
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <p className="text-xs text-muted-foreground">
                  {caseAwareLabel} · subtotal {formatCurrency(effectivePrice * qty)}
                  {casePack > 1 && (
                    <span className="ml-1 text-emerald-700 dark:text-emerald-400">
                      (steps of {casePack} for full cases)
                    </span>
                  )}
                </p>
                ...
                <Dialog open={guideOpen} onOpenChange={setGuideOpen}>
                  ...
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle>Save to order guide</DialogTitle>
                      <DialogDescription>
                        Create a new reusable order guide containing this product. You can later
                        reorder from any saved guide in one click.
                      </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-3 py-2">
                      <div className="space-y-2">
                        ...
                        <Input
                          id="guide-name"
                          placeholder="e.g. Monthly restroom restock"
                          value={guideName}
                          onChange={(e) => setGuideName(e.target.value)}
                        />
                      </div>
                      <div className="rounded-md border border-border/60 bg-muted/30 p-3 text-xs text-muted-foreground">
                        Adding: <span className="font-medium text-foreground">{product.name}</span>
                        <br />
                    ...
                    <DialogFooter>
                      ...
                      <Button onClick={handleSaveToGuide}>Save guide</Button>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </div>
                ...
                <table className="w-full text-sm">
                  <tbody>
                    ...
                    <tr className="bg-background">
                      <th className="px-4 py-2.5 text-left font-medium text-muted-foreground">Case Pack</th>
          ...
          <TabsContent value="sustainability">
            <Card className="p-6">
              ...
              <p className="mb-5 text-sm text-muted-foreground">
                Measured per case, compared to conventional virgin-fiber equivalents. These metrics
                roll up into your company-wide impact report.
              </p>
